                      ...
                      returnCheckedData = FALSE, ...) 
{
    ...
    if (!is.null(sci)) {
        ...
        if (nrow(sci) != 356) validSCI <- FALSE
        if (ncol(sci) != 88) validSCI <- FALSE
        if (!validSCI) {
            stop("error: invalid sci (must be data frame or matrix with 356 rows and 88 columns).")
        }
        probbaseV6 <- as.matrix(sci)
        probbaseV6Version <- probbaseV6[1,3]
    }
  ...
  if ( "i183o" %in% colnames(Input)) {
    ...
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "I"  ] <- 1
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "A+" ] <- 0.8
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "A"  ] <- 0.5
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "A-" ] <- 0.2
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "B+" ] <- 0.1
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "B"  ] <- 0.05
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "B-" ] <- 0.02
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "B -"] <- 0.02
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "C+" ] <- 0.01
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "C"  ] <- 0.005
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "C-" ] <- 0.002
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "D+" ] <- 0.001
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "D"  ] <- 5e-04
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "D-" ] <- 1e-04
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "E"  ] <- 1e-05
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == "N"  ] <- 0
    probbaseV6[,18:ncol(probbaseV6)][probbaseV6[,18:ncol(probbaseV6)] == ""   ] <- 0
    probbaseV6[1, 1:17] <- rep(0, 17)
    Sys_Prior <- as.numeric(probbaseV6[1, ])
    D <- length(Sys_Prior)
    HIV <- tolower(HIV)
    ...
    if (write && append == FALSE) {
        ...
        if (output == "extended") 
            header = c(header, as.character(causetextV6[, 2]))
    ...
    for (i in 1:N) {
        ...
        tmp <- DataCheck5(input.current, id=index.current, probbaseV5=probbaseV6, write=write)
        if (returnCheckedData) {
        ...
        subst.vector[probbaseV6[,6]=="N"] <- 0
        subst.vector[probbaseV6[,6]=="Y"] <- 1
        ...
        for (jj in 1:length(temp)) {
            temp_sub <- temp[jj]
            for (j in 18:D) {
                    prob[j - 17] <- prob[j - 17] * as.numeric(probbaseV6[temp_sub + 1, j])
            }
            if (sum(prob[1:3]) > 0) 
            ...
            if (sum(prob[65:71]) > 0)
                prob[65:71] <- prob[65:71]/sum(prob[65:71])
        }
        names(prob) <- causetextV6[, 2]
        prob_A <- prob[ 1: 3]
        prob_B <- prob[ 4:64]
        prob_C <- prob[65:71]
